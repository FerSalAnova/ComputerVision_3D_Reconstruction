...
def resBundleProjection_MulViews_less(Op, x1Data, x2Data, x3Data, x4Data, K_c, K_old, nPoints1,  nPoints2,  nPoints3):
    
    R_2, R_3, R_4, tvec_2, tvec_3, tvec_4, X_w_1, X_w_2, X_w_3 = extract_optimized_parameters_MulViews_less(Op, nPoints1, nPoints2, nPoints3)
    ...
    residuals = np.hstack([residuals_x1.flatten(), residuals_x2.flatten(), residuals_x3.flatten(),residuals_x4.flatten()])

    return residuals

def create_optimization_parameters_MulViews_less(T1, T2, T3, triangulated_points_1, triangulated_points_2, triangulated_points_3):
    # Extract rotation and translation
    R_1 = T1[0:3, 0:3]
    t_1 = T1[0:3, 3]
    t_1 = cartesian_to_angle_vector(t_1)
    ...
    # Create optimization parameter vector
    Op_initial = np.concatenate([rotation_vec_1, rotation_vec_2, rotation_vec_3,t_flat_1, t_flat_2, t_flat_3, X_flat_1, X_flat_2 , X_flat_3])
    Op_initial = np.real(Op_initial)  # Ensure real values

    return Op_initial


def extract_optimized_parameters_MulViews_less(Op, nPoints1, nPoints2, nPoints3):
    # Extract optimized parameters
    optimized_R_vec_1 = Op[:3]  
    optimized_R_vec_2 = Op[3:6]
    optimized_R_vec_3 = Op[6:9]  

    optimized_t_1 = Op[9:11]      
    optimized_t_2 = Op[11:14]
    optimized_t_3 = Op[17:20]           
    
    optimized_X_w_1 = Op[20:20 + 3 * nPoints1].reshape(nPoints1, 3).T  
    optimized_X_w_2 = Op[20 + 3 * nPoints1:20 + 3 * nPoints1 + 3*nPoints2 ].reshape(nPoints2, 3).T 
    optimized_X_w_3 = Op[20 + 3 * nPoints1 + 3*nPoints2:20 + 3* nPoints1 + 3*nPoints2+3*nPoints3].reshape((nPoints3-1), 3).T   
    ...
    return optimized_R_1, optimized_R_2, optimized_R_3, optimized_t_1,optimized_t_2,optimized_t_3,  optimized_X_w_1, optimized_X_w_2, optimized_X_w_3
...
if __name__ == '__main__':
    ...
    common_kp1_6_2 = get_common_keypoints([kp1_6_match, kp1_2_match])
    common_kp2_2 = get_common_kp2(kp1_2_match, kp2_2_match, common_kp1_6_2)
    
    common_kp1_6_4 = get_common_keypoints([kp1_6_match, kp1_4_match])
    common_kp2_4 = get_common_kp2(kp1_4_match, kp2_4_match, common_kp1_6_4)
    
    common_kp1_6_5 = get_common_keypoints([kp1_6_match, kp1_5_match])
    common_kp2_2 = get_common_kp2(kp1_5_match, kp2_5_match, common_kp1_6_5)
    
    common_kp1_6_7 = get_common_keypoints([kp1_6_match, kp1_7_match])
    common_kp2_7 = get_common_kp2(kp1_7_match, kp2_7_match, common_kp1_6_7)
    
    common_kp1_6_10 = get_common_keypoints([kp1_6_match, kp1_10_match])
    common_kp2_10 = get_common_kp2(kp1_10_match, kp2_10_match, common_kp1_6_10)
    
    common_kp1_6_11 = get_common_keypoints([kp1_6_match, kp1_11_match])
    common_kp2_11 = get_common_kp2(kp1_11_match, kp2_11_match, common_kp1_6_11)
    
    common_kp1_6_13 = get_common_keypoints([kp1_6_match, kp1_13_match])
    common_kp2_13 = get_common_kp2(kp1_13_match, kp2_13_match, common_kp1_6_13)
    
    common_kp1_6_5 = get_common_keypoints([kp1_6_match, kp1_5_match])
    common_kp2_5 = get_common_kp2(kp1_5_match, kp2_5_match, common_kp1_6_5)
    ...
    for i in range(len(common_kp1_6_4)):
        for a in range(len(kp1_6_match)):
            if np.all(common_kp1_6_4[i] == kp1_6_match[a]):
                threeDPoints_4.append(optimized_X.T[a])

    threeDPoints_4 = np.array(threeDPoints_4)
    
    threeDPoints_5 = []

    for i in range(len(common_kp1_6_5)):
        for a in range(len(kp1_6_match)):
            if np.all(common_kp1_6_5[i] == kp1_6_match[a]):
                threeDPoints_5.append(optimized_X.T[a])

    threeDPoints_5 = np.array(threeDPoints_5)
    
    threeDPoints_7 = []

    for i in range(len(common_kp1_6_7)):
        for a in range(len(kp1_6_match)):
            if np.all(common_kp1_6_7[i] == kp1_6_match[a]):
                threeDPoints_7.append(optimized_X.T[a])

    threeDPoints_7 = np.array(threeDPoints_7)
    
    threeDPoints_10 = []

    for i in range(len(common_kp1_6_10)):
        for a in range(len(kp1_6_match)):
            if np.all(common_kp1_6_10[i] == kp1_6_match[a]):
                threeDPoints_10.append(optimized_X.T[a])

    threeDPoints_10 = np.array(threeDPoints_10)
    
    threeDPoints_11 = []

    for i in range(len(common_kp1_6_11)):
        for a in range(len(kp1_6_match)):
            if np.all(common_kp1_6_11[i] == kp1_6_match[a]):
                threeDPoints_11.append(optimized_X.T[a])

    threeDPoints_11 = np.array(threeDPoints_11)
    
    threeDPoints_13 = []

    for i in range(len(common_kp1_6_13)):
        for a in range(len(kp1_6_match)):
            if np.all(common_kp1_6_13[i] == kp1_6_match[a]):
                threeDPoints_13.append(optimized_X.T[a])

    threeDPoints_13 = np.array(threeDPoints_13)
    ...
    R_4, t_4, inliers = ransac_pnp(threeDPoints_4, np.array(common_kp2_4), K, max_iterations=1000, threshold=1.0)

    success, R_4, t_4, inliers = cv2.solvePnPRansac(
    threeDPoints_4, np.array(common_kp2_4), K, dist_coeffs)

    R_4, _ = cv2.Rodrigues(R_4)
    ...
    R_5, t_5, inliers = ransac_pnp(threeDPoints_5, np.array(common_kp2_5), K, max_iterations=1000, threshold=1.0)

    success, R_5, t_5, inliers = cv2.solvePnPRansac(
    threeDPoints_5, np.array(common_kp2_5), K, dist_coeffs)

    R_5, _ = cv2.Rodrigues(R_5)
    ...
    R_7, t_7, inliers = ransac_pnp(threeDPoints_7, np.array(common_kp2_7), K, max_iterations=1000, threshold=1.0)

    success, R_7, t_7, inliers = cv2.solvePnPRansac(
    threeDPoints_7, np.array(common_kp2_7), K, dist_coeffs)

    R_7, _ = cv2.Rodrigues(R_7)
    ...
    R_10, t_10, inliers = ransac_pnp(threeDPoints_10, np.array(common_kp2_10), K, max_iterations=1000, threshold=1.0)

    success, R_10, t_10, inliers = cv2.solvePnPRansac(
    threeDPoints_10, np.array(common_kp2_10), K, dist_coeffs)

    R_10, _ = cv2.Rodrigues(R_10)
    ...
    R_11, t_11, inliers = ransac_pnp(threeDPoints_11, np.array(common_kp2_11), K, max_iterations=1000, threshold=1.0)

    success, R_11, t_11, inliers = cv2.solvePnPRansac(
    threeDPoints_11, np.array(common_kp2_11), K, dist_coeffs)

    R_11, _ = cv2.Rodrigues(R_11)

    T_11 = np.eye(4)
    T_11[:3, :3] = R_11
    T_11[:3, 3] = -t_11.flatten()

    print(T_11)
    
    R_13, t_13, inliers = ransac_pnp(threeDPoints_13, np.array(common_kp2_13), K, max_iterations=1000, threshold=1.0)

    success, R_13, t_13, inliers = cv2.solvePnPRansac(
    threeDPoints_13, np.array(common_kp2_13), K, dist_coeffs)

    R_13, _ = cv2.Rodrigues(R_13)

    T_13 = np.eye(4)
    T_13[:3, :3] = R_13
    T_13[:3, 3] = -t_13.flatten()
    ...
    drawRefSystem(ax, T_5, '-', 'C_5')
    drawRefSystem(ax, T_7, '-', 'C_7')
    drawRefSystem(ax, T_10, '-', 'C_10')
    drawRefSystem(ax, T_11, '-', 'C_11')
    drawRefSystem(ax, T_13, '-', 'C_13')
    drawRefSystem(ax, T_4, '-', 'C_4')
    ...
    # ax.scatter(trian_points_15[:, 0], trian_points_15[:, 1], trian_points_15[:,2], marker='o', color='b')
    ...
    # Op_initial = create_optimization_parameters_MulViews_less(T_old, T_15, T_9,  trian_points_old,trian_points_15,trian_points_9)

    # Op_initial = np.real(Op_initial)
    
    # # Run least squares optimization
    # result = least_squares(resBundleProjection_MulViews_less, Op_initial, args=(np.array(kp1_0_match), np.array(kp2_0_match), np.array(kp2_15_match), np.array(kp2_9_match), K,K_old, nPoints1,  nPoints3,  nPoints8), max_nfev=80)

    # # Extract optimized parameters
    # optimized_R_0,  optimized_R_15,  optimized_R_9, optimized_t_0,optimized_t_15, optimized_t_9, optimized_X_w_0,  optimized_X_w_15,   optimized_X_w_9 = extract_optimized_parameters_MulViews_less(result.x, nPoints1,  nPoints3, nPoints8)
